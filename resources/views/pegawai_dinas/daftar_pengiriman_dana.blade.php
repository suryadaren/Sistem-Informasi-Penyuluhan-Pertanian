                                ...
                                <div class="datatable-dashv1-list custom-datatable-overright">
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" >
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$no++}}</td>
                                                <td>{{$d->proposal_dana->draft_programa->user->nama}}</td>
                                                <td>{{$d->proposal_dana->draft_programa->user->desa}}</td>
                                                <td>{{$d->proposal_dana->draft_programa->user->kecamatan}}</td>
                                                <td>Rp. {{$d->jumlah}},-</td>
                                                <td>{{$d->updated_at->format('Y-m-d')}}</td>
                                                <td>{{$d->status}}</td>
                                                <td class="datatable-ct"><a href="/pegawai_dinas/detail_pengiriman_dana/{{$d->id}}" class="btn btn-primary">lihat</a></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
